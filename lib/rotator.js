...
async function rotate(bannedDomain, triggerSource = 'api') {
  ...
  try {
    ...
    if (!wasActive) {
      ...
      // Remove from RT stream even if not active — lander slot should be freed
      if (fromDomain.redtrack_stream_id && fromDomain.redtrack_lander_id) {
        try {
          const apiKey = process.env.REDTRACK_API_KEY;
          if (apiKey) {
            ...
            if (stream) {
              ...
              await putStream(apiKey, fromDomain.redtrack_stream_id, { ...stream, landings: remaining });
              console.log(`[rotator] Removed standby lander ${fromDomain.redtrack_lander_id} from RT stream ${fromDomain.redtrack_stream_id}`);
            }
          }
        } catch (err) {
          console.error('[rotator] Failed to remove standby lander from RT stream:', err.message);
        }
      }
