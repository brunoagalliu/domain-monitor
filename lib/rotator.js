...
async function rotate(bannedDomain, triggerSource = 'api') {
  ...
  let fromDomain, memberships;
  {
    ...
    try {
      ...
      const { rows: m } = await client.query(
        `SELECT df.funnel_id, df.status, df.redtrack_lander_id AS funnel_lander_id,
                f.redtrack_stream_id, f.auto_rotate, f.name AS funnel_name
  ...
  for (const membership of memberships) {
    if (membership.status !== 'active') {
      ...
      const bannedLanderId = membership.funnel_lander_id || fromDomain.redtrack_lander_id;
      await removeFromStream(membership.redtrack_stream_id, bannedLanderId);
      await pool.query(
    ...
    try {
      ...
      const { rows: [nextDomain] } = await client.query(
        `SELECT d.*, df.redtrack_lander_id AS funnel_lander_id FROM domains d
         JOIN domain_funnels df ON df.domain_id = d.id
         WHERE df.funnel_id = $1
           AND df.status = 'standby'
           AND d.rotator_status != 'banned'
           AND d.is_active = true
         ORDER BY (COALESCE(df.redtrack_lander_id, d.redtrack_lander_id) IS NOT NULL) DESC, d.rotator_priority DESC, d.created_at ASC
         LIMIT 1`,
        ...
        if (membership.redtrack_stream_id) {
          try {
            ...
            if (apiKey) {
              ...
              if (stream) {
                const remaining = (stream.landings || []).filter(l => String(l.id) !== String(bannedLanderId));
      ...
      // Update RT stream using per-funnel lander IDs
      const bannedLanderId = membership.funnel_lander_id || fromDomain.redtrack_lander_id;
      const nextLanderId   = nextDomain.funnel_lander_id || nextDomain.redtrack_lander_id;
      let rtWarning = null;
      if (membership.redtrack_stream_id && nextLanderId) {
        try {
          await rotateInStream(membership.redtrack_stream_id, bannedLanderId, nextLanderId);
        } catch (err) {
