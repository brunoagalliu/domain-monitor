...
// Keep only Safe Browsing databases and Preferences — delete everything else.
// Chrome accumulates GBs of cache, crash dumps, and session data over time.
function purgeChromeCache() {
  const keep = new Set(['Safe Browsing', 'Preferences']);
  const defaultDir = path.join(CHROME_PROFILE, 'Default');
  try {
    if (fs.existsSync(defaultDir)) {
      for (const entry of fs.readdirSync(defaultDir)) {
        if (!keep.has(entry)) {
          try { fs.rmSync(path.join(defaultDir, entry), { recursive: true, force: true }); } catch {}
        }
      }
    }
    // Also clear top-level profile dirs (shader caches, crash reports)
    for (const entry of fs.readdirSync(CHROME_PROFILE)) {
      if (entry !== 'Default') {
        try { fs.rmSync(path.join(CHROME_PROFILE, entry), { recursive: true, force: true }); } catch {}
      }
    }
  } catch {}
}
