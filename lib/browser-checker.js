...
// Delete only the HTTP/JS caches from the Chrome profile. Everything else
// (IndexedDB, Network state, Safe Browsing DB, etc.) must be left intact.
function purgeChromeCache() {
  const defaultDir = path.join(CHROME_PROFILE, 'Default');
  for (const dir of ['Cache', 'Code Cache']) {
...
class BrowserChecker {
  ...
  async init() {
    ...
    this.context = await this._launchWithReset(launchOpts);
    console.log(`🌐 Browser checker ready (${USE_XVFB ? 'headed+Xvfb' : 'headless'} Playwright Chromium, Safe Browsing active)`);
  }

  async _launchWithReset(launchOpts) {
    for (let attempt = 0; attempt <= 1; attempt++) {
      if (attempt === 1) {
        // Profile is corrupted — nuke it entirely and start fresh.
        console.warn('[browser] resetting Chrome profile and retrying');
      ...
      let ctx;
      try {
        ctx = await chromium.launchPersistentContext(CHROME_PROFILE, launchOpts);
      } catch (err) {
        if (attempt === 0) { console.warn('[browser] launch failed:', err.message.split('\n')[0]); continue; }
        throw err;
      }

      const [defaultPage] = ctx.pages();
      if (defaultPage) await defaultPage.close();
      if (USE_XVFB) await new Promise(r => setTimeout(r, 2000));

      // Health check: Chromium can launch but crash before the first newPage().
      // Verify it's actually usable before returning.
      try {
        const hp = await ctx.newPage();
        await hp.close();
        return ctx;
      } catch (err) {
        console.warn('[browser] health check failed:', err.message.split('\n')[0]);
  ...
  async checkDomains(domains) {
    ...
    // allSettled so one page crash doesn't abort the entire batch
