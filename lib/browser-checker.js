...
class BrowserChecker {
  ...
  async startXvfb() {
    if (this.xvfb) { this.xvfb.kill(); this.xvfb = null; }
  ...
  async init() {
    ...
    try {
      ...
      console.warn('[browser] launch failed, resetting profile + Xvfb:', err.message.split('\n')[0]);
  ...
  // Close only the Chrome context, keep Xvfb running so the next init is faster.
  async closeContext() {
    if (this.context) {
      this._closing = true;
