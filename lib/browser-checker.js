...
class BrowserChecker {
  ...
  async init() {
    ...
    this.context = await chromium.launchPersistentContext(CHROME_PROFILE, {
      ...
      headless: !USE_XVFB,
      timeout: 60000,
      // Prevent Playwright from disabling Safe Browsing's phishing detection
      ignoreDefaultArgs: ['--disable-client-side-phishing-detection'],
      args: [
        '--no-sandbox',
    ...
    console.log(`🌐 Browser checker ready (${USE_XVFB ? 'headed + Xvfb' : 'headless'}, Safe Browsing active)`);
  }
  ...
  async checkDomains(domains) {
    ...
    try {
      ...
      if (/has been closed|Target page|browser has been closed/i.test(err.message)) {
        console.log('🔄 Browser context closed, reinitialising...');
        this.context = null;
        await this.init();
