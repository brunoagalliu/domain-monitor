...
class BrowserChecker {
  ...
  async init() {
    ...
    if (USE_XVFB && !this.xvfb) await this.startXvfb();
    ...
    this.context.on('close', () => {
      if (!this._closing) console.error('[browser] ⚠️ Chrome context closed unexpectedly');
      this.context = null;
      this.page = null;
    });
  ...
  async checkDomain(domain) {
    if (!this.context || !this.page) throw new Error('Browser context not ready');
    ...
    console.log(`[browser] → navigating to https://${domain}`);

    try {
      await page.goto(`https://${domain}`, { waitUntil: 'commit', timeout: TIMEOUT });

      const finalUrl = page.url();
      console.log(`[browser] ← ${domain}: finalUrl=${finalUrl} (${Date.now() - t}ms)`);

      if (finalUrl.startsWith('chrome-error://') || finalUrl.startsWith('chrome://interstitialpage')) {
        console.log(`[browser] 🚨 ${domain}: DANGEROUS (chrome-error interstitial)`);
        return { domain, status: 'dangerous' };
      }
      console.log(`[browser] ✅ ${domain}: safe`);
      return { domain, status: 'safe' };
    } catch (err) {
      const msg = err.message || '';
      console.log(`[browser] ⚠️ ${domain}: error — ${msg.split('\n')[0]} (${Date.now() - t}ms)`);
      if (/ERR_BLOCKED_BY_CLIENT|ERR_BLOCKED_BY_ADMINISTRATOR|ERR_UNSAFE_REDIRECT/i.test(msg)) {
        console.log(`[browser] 🚨 ${domain}: DANGEROUS (blocked by client)`);
        return { domain, status: 'dangerous' };
      }
      return { domain, status: 'safe' };
    } finally {
