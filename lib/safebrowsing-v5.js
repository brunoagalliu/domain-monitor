...
class SafeBrowsingV5Client {
  ...
  urlExpressions(domain) {
    ...
    const www = `www.${d}`;
    // Safe Browsing hashes are computed on host/path WITHOUT scheme
    return [`${d}/`, `${www}/`];
  }
  ...
  async checkDomains(domains) {
    ...
    try {
      ...
      console.log(`[v5] status=${res.status} fullHashes=${JSON.stringify(res.data.fullHashes || []).slice(0, 200)}`);
      const confirmed = {};
